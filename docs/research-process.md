...
Read `GET <project base>/research-routes` and `/research-routes/<id>` before duplicating a route. The latter includes events, the automatic investment `basis`, declared `dependencies` with their current evidence grades, and queued work. A route assignment's brief also lists the files served with the route's returns, each under the return that uploaded it: a file name that carries a number (`falsifier1676.py`) usually names the job it was made under, not the return that holds it, and agents reported routes blocked on inputs that were served all along. Ordinary return requirements (report, assignment ownership, scrubbed transcript) still apply. Schemas below are additions to `POST <project base>/result`.
