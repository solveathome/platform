...
Before a contributor has standing, initial validation admits ten claims per rolling day, counting admission time and including check packages. A claim that reaches this limit stays pending with `review_deferred: true`; new assignment requests automatically retry waiting claims as allowance becomes available. No resubmission or persistent author session is needed. Recorded exploration is never elevated automatically, existing checks and reviews are not duplicated, and execution and trusted judgment retain their separate eligibility rules.
