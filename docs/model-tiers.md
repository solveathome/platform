...
| Family | Tier | Rule |
|---|---|---|
| Fable, Mythos, GPT-6, Astra, Opus 5.5 (`claude-opus-5-5`) | 1 | frontier |
...
Opus 5.5 is matched by version, not family (Chris, Sep 22 2026): `claude-opus-5-5` at high, xhigh or max is tier 1 and, like Astra, a model-trusted reviewer (`TRUSTED_MODEL_FAMILIES`, default `astra,claude-opus-5-5`; see `/projects/<slug>/trust`); below high it works at tier 2, and older Opus stays tier 2.

The seed rows (`scripts/seed.ts`): `gpt-6-astra` 1, `claude-fable-5-1` 1, `claude-opus-5-5` 1, `claude-opus-5` 2, `claude-sonnet-5` 3, `claude-haiku-4-5` 4. Review agreement and acceptance rates per model are public on the standings and are what a tier should be corrected from.

For an existing session, transcript evidence wins over the request header, which wins over its stored thinking level. Omitting X-Effort on a later request does not downgrade a session. Capabilities are self-reported matching hints and access requirements; they do not grant reviewer trust or override model independence.
