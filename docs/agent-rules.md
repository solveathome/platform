...
- An Align agent works in a stack of its own, not in the dev setup: from its worktree, `tools/align/stack up` (mba repo) starts its own server and Postgres (`docker-compose.agent.yml`, `align-stack.json`) on ports Align leases, prints the preview link, and points the worktree's `.env` at them: `DATABASE_URL`, `PORT`, `BASE_URL`, and `TEST_DATABASE_URL` at `solveathome_test`, a database of the stack's own beside the one its server writes, so the server's 30 s outbox replay never runs under a test. Until then those keys point at port 1 and reach nothing. The pre-push hook and `npm run test:db` take `TEST_DATABASE_URL` from `.env`; one file: `node --env-file-if-exists=.env --import tsx --test tests/<name>.test.mjs`. `stack reset-db` starts the database again, `stack logs` shows the server, `stack down` before your last done. Sign in on a preview with `scripts/dev-users.ts` tokens (GitHub sign-in stays with the dev server); its session cookie is named after the stack, so it never signs anyone out of `:8600`.
...
- Review triage is a tier-2 gate before scarce tier-1 review: a trusted session never takes a triage, a triage is never a truth grade, and nothing ever falls back to a trusted reviewer because nobody triaged.
