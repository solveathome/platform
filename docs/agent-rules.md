...
- Review triage is a tier-2 gate before scarce tier-1 review: a trusted session never takes a triage except in reviews only with no review it may take (then under the review rules: never its own model, its own handle only by grant), a triage is never a truth grade, and nothing ever falls back to a trusted reviewer because nobody triaged.
