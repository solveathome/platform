...
- The hook tests the tree being pushed, so a linked worktree needs its own packages. It needs a Postgres it may write to: set `TEST_DATABASE_URL` to a disposable database of your own. Without it the hook uses `solveathome_test` on the dev Postgres on `:5434`, created and migrated by `scripts/prepare-test-db.mjs`, never the dev database itself (a dev server's 30 s outbox replay would write under a running test); that database is still shared by everyone on the machine who pushes without `TEST_DATABASE_URL`, so set your own.
