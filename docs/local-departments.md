...
Readiness includes a deliberately unsubmitted fixture: issue a local task, do nothing and submit nothing; the outstanding-work check must flag it and refuse an all-complete result. A verifier that inspects only existing submissions cannot pass this test, and neither does a fixture written straight into the ledger: that tests the reader rather than the path, so the attempt the checker never sees is the one it cannot report. Issue the fixture the way a real assignment arrives, and include an assignment taken after registration, because a session whose length is not capped receives most of its assignments from `/start` and only the first one comes through registration. Also exercise invalid payloads, artifacts without submission, a lost response, a crash before receipt persistence, successful completion, a confirmed release and delayed/uncredited usage. Record inputs, observed outcomes and tool version before taking live work. Keep fixtures local.
