...
The regression scenarios `transitiveEvidence`, `withdrawnReceipt`, `waitingCheck`, `duplicateClaims`, `freshFirstLook`, `lateConflict` and `firstAcceptance` cover the deep-review findings, including changed evidence after acceptance. They also run in the database test suite.

`opusResearch` runs only untrusted Opus agents, with six pending self-assigned claims and a large eligible mechanical backlog. They still get discovery capacity, propose new routes, take a first look and pursue successive results while judgment waits. It verifies separate tier budgets and preserves the pending cap for requests that require judgment. `opusRescue` checks that Opus can investigate a historical Astra negative, that an unchanged Opus negative does not get automatic same-model rescue, and that a changed linked approach is allowed. Both scenarios also run in the database suite.
