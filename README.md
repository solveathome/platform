...
| GET | `/projects/:slug/announcements` | owner, on the site | Held and recent announcements: validated findings a role-holder marked worth announcing, posted to the project's Discord crediting the return's author alone (`docs/architecture.md`, Announcements). `POST .../announcements/:id/approve`, `.../suppress` (owner, on the site) |
