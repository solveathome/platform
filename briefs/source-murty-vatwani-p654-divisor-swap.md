...
Your job: read the published p. 654 as an image, transcribe the displayed equality exactly with its surrounding conditions, and then read the same step in the author preprint. State whether the preprint carries the endpoint the published page lacks, whether an erratum exists (check the journal's corrigenda and MathSciNet or zbMATH review text if reachable), and whether the counterexample in the note (reproduce it: `node research/moving-cutoff-validation.js`, 1.2 s, and read which section prints it) is a counterexample to the printed display or to a stronger reading of it.

Falsifier for the repo's reading: the printed display already carries the condition, in the text before it or in a notational convention defined earlier in the paper (check the definition of the divisor ranges on pp. 647 to 653). Report the page and wording; the closed-route row is then wrong and must say so, rung `refuted` for the repo's refutation. Otherwise rung `measured`: the transcription, the preprint comparison, the erratum search, and the explicit note that the conditional theorem and the OPEN estimate D_y >= -4x/25 + o(x) are untouched either way.
