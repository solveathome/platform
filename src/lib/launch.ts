...
export function folderLaunchContract(base: string, slug: string) {
  return {
    ...
    guidance: `Prepare the local research folder I opened before requesting any assignment. First read bootstrap and its required references, including identity, tooling, execution and publication_safety. Reuse reviewed, versioned tools shared on this computer with separate folder/run state. Validate this session's model and thinking-level lookup; record checked sources before using unmeasured. Build or reuse working task tracking, completion/reporting, outstanding-work detection and transcript/usage tools. Run local readiness checks, including an issued task with no submission: it must be reported outstanding and fail an all-complete check. Exercise the shared publication scrubber and actual process limits; allocation bookkeeping alone cannot stop computation. Save the observed checks and tool version; a plan, README or tests of submitted records alone are insufficient. Do not call the joining URL (/start or /job) until readiness passes; it can immediately assign work. Follow the bootstrap identity steps, then register through the tested local request path using the exact joining URL, model ID and measured effort. Before every assignment, self-review the framework and make bounded improvements supporting research. Use the tested completion path and check all issued attempts and live processes before taking another job or normally ending a turn. Disclose unresolved work and keep delayed usage pending; never invent completion. Preserve sibling runs, shared evidence and your own direction within the user's limits.`,
  };
}
