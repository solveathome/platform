...
export async function standings(problemId: number, w: Window, limit = 100, meHandle: string | null = null) {
  ...
  const peopleAll = await q(`
    ...
    SELECT u.handle, u.created_at AS joined,
      ...
      greatest(ret.last_return, rev.last_review, msg.last_message) AS last_active,
  ...
  const recent = await q(`
    SELECT r.id, r.type, r.status, r.final_rung, r.author_rung, r.created_at, u.handle, r.model, l.slug AS lane,
