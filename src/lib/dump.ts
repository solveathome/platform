...
export const DUMP_TABLES: Record<string, string> = {
  ...
  jobs:      `SELECT j.id, p.slug AS project, l.slug AS lane, j.type, j.title, j.title_before, j.brief_md, j.git_ref, j.compute_hint, j.budget_hours, j.min_tier, j.quorum, j.parent_return_id, j.purpose, j.research_stage, j.research_route_id, j.research_revision, j.research_source_return_id, j.step_check_of, j.step_checked_through, j.evidence_return_id, j.check_wait_expired_at, j.avoid_model, j.required_tools, j.required_sources, j.status, u.handle AS assigned_to, j.assigned_at, j.expires_at, j.created_at FROM jobs j JOIN problems p ON p.id = j.problem_id LEFT JOIN lanes l ON l.id = j.lane_id LEFT JOIN users u ON u.id = j.assigned_to ORDER BY j.id`,
  ...
  reviews:   `SELECT rv.id, rv.return_id, rv.review_job_id, u.handle, rv.department_id,rv.run_id,rv.model, rv.provider, rv.verdict, rv.rung, rv.notes_md, rv.trusted, rv.verification, rv.verification_receipt_id, rv.verification_sufficiency_md, rv.verification_conflict_through, rv.verification_conflict_resolution_md, rv.needs_reassessment, rv.weight, rv.agreed_with_outcome, rv.announce, rv.announce_md, rv.created_at FROM reviews rv JOIN users u ON u.id = rv.user_id ORDER BY rv.id`,
...
const PROSE = new Set(["status_md", "brief_md", "report_md", "patch", "transcript", "notes_md", "body_md", "question", "verdict", "contribution_md", "prior_art_md", "uncertainty_md", "evidence_md", "observed", "verification_sufficiency_md", "verification_conflict_resolution_md", "announce_md"]);
