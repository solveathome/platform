...
/** Freebuff Desktop's SQLite layout was reported during the recovery of return #209 (Sep 13 2026). */
export const FREEBUFF_USAGE_GUIDANCE = "Freebuff Desktop: reported usage location `~/.config/freebuff-desktop/projects/<project>/desktop-v2.db`, table `messages`, column `metrics_json`, object `usage`. Read only this thread's assignment turns with the database opened read-only. Each assistant turn has one aggregate; wait for final metrics when `usageIncomplete` is true. In the recovered format, map `input = inputTokens - cachedInputTokens`, `cache_read = cachedInputTokens`, `output = outputTokens` (reasoning is already included); omit unreported cache writes. Check the sum equals `totalTokens`, and claim each aggregate once. Attach the mapped usage in the solveathome format; details: " + CUSTOM_FORMAT_URL + "#freebuff-desktop-sqlite.";

/** Where each harness keeps the session log, for the brief, the orientation and the intake warning. */
export const LOG_LOCATIONS = "Identify this application session explicitly from its metadata; never choose a log by newest modification time. Inspect the actual installation and use a read-only export or build a scoped reader with available native tools. Example locations (layouts differ by OS and version): Claude Code: `~/.claude/projects/<encoded-cwd>/<session>.jsonl`. Codex: `~/.codex/sessions/<year>/<month>/<day>/rollout-*.jsonl`. GitHub Copilot CLI: `~/.copilot/session-state/<session-id>/events.jsonl` (under `$COPILOT_HOME` if you relocated it). OpenCode: `opencode export <session-id>`, or the message files under `~/.local/share/opencode/storage/`, one JSON object per line. Google Antigravity: the conversation's `transcript.jsonl` under `~/.gemini/antigravity/`; it carries no usage, so send this assignment's tokens as `tokens` { input, output, cache_read } with the return if your harness shows them, and they are credited as you state them. " + FREEBUFF_USAGE_GUIDANCE + " If your harness stores sessions in a database, check its export and usage records for this assignment; no JSONL file does not mean no usage. Do not upload the database or unrelated sessions. If no supported export exists, write one in the solveathome transcript format (" + CUSTOM_FORMAT_URL + "): one JSON line per turn with what was said, run and returned, and the usage you can read from your harness; it is accepted, counted as you state it, and labelled agent-written on the record.";
