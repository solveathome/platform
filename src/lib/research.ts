...
export async function routeContext(id: number): Promise<any> {
  ...
  const basis = await q(`WITH RECURSIVE basis AS (
      ...
      WHERE j.research_route_id=$2 AND j.research_stage IN ('first_look','pursue')
...
export async function recordResearch(ret: any, job: any, report: ResearchReport | null): Promise<any> {
  if (job?.research_route_id && ['first_look', 'pursue', 'rescue'].includes(job.research_stage) && !report) bad('this route assignment requires research with outcome, evidence and its next step or obstacle');
  ...
  if (report.proposal) {
    ...
    // No cap on new routes (Chris, Sep 23 2026: "tasks should never have an upper cap in how many jobs it can do"). The ten-a-day
    // cap left an agent running for days holding a rescue it could not submit (job #2852, #sah-no-capped-assignments).
...
export async function reconsiderDependents(returnId: number, status: string): Promise<number[]> {
  ...
  const descendants = await q(`WITH RECURSIVE edges AS NOT MATERIALIZED (
      SELECT r.id AS dependent,j.research_source_return_id AS premise FROM returns r JOIN jobs j ON j.id=r.job_id
        WHERE j.research_stage IN ('first_look','pursue') AND r.research_route_id=j.research_route_id
  ...
  for (const route of affected) {
    ...
    await q(`UPDATE jobs SET status='expired' WHERE research_route_id=$1 AND research_stage IN ('first_look','pursue','rescue') AND status='queued'`, [route.id]);
  }
  return ids;
}
