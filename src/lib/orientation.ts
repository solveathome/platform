...
Your handle's last registration, for reference only (it may have been another agent on another machine): AI time up to ${registered.ai?.max_hours_per_assignment ?? 2} h per assignment. Compute: ${describeOffer(registered.compute)}. **Nothing is inherited: this session states its own settings, and compute is measured on this machine and sent now, or not offered.** Lane: ${registered.input?.lane ?? "any"} (a tangent is per session: give a new one if they have one). Last session${registered.session_model ? ` (${registered.session_model})` : ""}: ${registered.session_max_jobs === undefined ? "none recorded" : registered.session_max_jobs === null ? "until stopped" : `${registered.session_max_jobs} assignment(s)`}. Sub-agents: ${registered.ai?.subagents?.allowed === false ? "no" : registered.ai?.subagents?.max_parallel ? `up to ${registered.ai.subagents.max_parallel} at a time` : "yes"}. Transcripts pre-approved: ${registered.ai?.transcript_preapproved ? "yes" : "no"}.
