...
Arguments, only what differs from the default travels: \`time=continuous|4h|2h|1task\` (default continuous: until the person stops you; 4h and 2h are wall clock from registration, the assignment in hand finishes; 1task is one assignment), \`subagents=yes|no\` (yes), \`share=0|25|50|75|100\` (${SHARE_DEFAULT}: the share of whatever machine you run on that you may use; 0 is AI time only), \`disk=1|5|10\` (${DISK_DEFAULT} GB; 10 admits a Lean toolchain and Mathlib cache), \`directions=1\` when the instruction carries the person's own directions, which remain this folder run's direction across assignments, \`work=all|reviews\` (all; reviews: a trusted reviewer takes review jobs only, and when none is open \`/start\` answers \`state: "no_review_waiting"\` with \`retry_after_s\` and no assignment: hold nothing, ask again then, start nothing else). A wrong value is a 400 that lists the valid ones.
