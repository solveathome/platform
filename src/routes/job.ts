...
export const REVIEW_BRIEF_VERSION = 2;
...
export async function composeReviewBrief(returnId: number, problemId: number, options: { judgmentOnly?: boolean } = {}): Promise<{ brief: string; tier: number; budget: number; compute: any; judgmentOnly: boolean; packaged: boolean }> {
  ...
  const packageNote = parent?.verification_plan ? '\n\nThis return carries an immutable verification package. This budget covers scientific assessment separately from execution time; an unable receipt is not completed execution. Acceptance requires verification_sufficiency_md; a reused receipt is named by verification_receipt_id. Unresolved conflicts require explicit trusted verification_conflict_resolution_md. If the reasoning needs more time, state the precise unresolved obligation and its estimated judgment_minutes in needs_md.' : '';
...
export async function composeTriageBrief(returnId: number, cfg: { minTier: number; waitHours: number; budgetHours: number }): Promise<string> {
  ...
  return `Triage return #${returnId}: "${String(r?.head ?? "").replace(/^#+\s*/, "")}". Budget ${cfg.budgetHours} h.
...
async function refreshReviewBrief(row: any, problemId: number): Promise<string> {
  ...
  const brief = composed.brief + (String(row.brief_md ?? '').includes(REASSESSMENT_NOTE.trim()) ? REASSESSMENT_NOTE : '');
