...
job.post("/result", bearer, project, async (req: any, res) => {
  ...
  const warnings = [...patchWarning, ...ledgerWarn, ...omissionWarn, ...(stray.length ? [`recipe_md names ${stray.length} sha256 value(s) that are neither in hashes, nor among your or cited files, nor a served document: ${stray.map((x: string) => x.slice(0, 12) + "…").join(", ")}. If one is an expected output hash, put it in hashes too; if it is a typo, a reviewer's rerun will not match.`] : [])];
...
export async function spawnReviews(returnId: number, problemId: number, laneId: number | null, n: number): Promise<void> {
  ...
  const parent = await one<{ type: string; paper_slug: string | null; revision_path: string | null; recipe_md: string | null; target: any; job_budget: string | null; job_compute: any; transcript_head?: string; model: string; handle: string; author_tier: number | null; problem_id: number; has_patch?: boolean; patch_scripts?: boolean; patch?: string | null; revision_sha?: string | null; transcript_omitted?: any }>(
    `SELECT r.type, r.paper_slug, r.revision_path, r.recipe_md, r.target, j.budget_hours AS job_budget, j.compute_hint AS job_compute, r.model, u.handle, mt.tier AS author_tier, r.problem_id, left(r.transcript, 24) AS transcript_head, r.patch, r.revision_sha, r.transcript_omitted, (r.patch IS NOT NULL) AS has_patch, (r.patch ~ '(^|\n)(\+\+\+|---) [^\n]*\.(js|mjs|cjs|ts|py|sh|c|h|cpp|rs|go|jl|lean|sql)(\s|$)') AS patch_scripts
...
async function returnPage(req: any, res: any): Promise<void> {
  ...
  const reviews = await q(`SELECT rv.id, rv.verdict, rv.rung, rv.reject_reason, rv.notes_md, rv.also_fix, rv.weight, rv.created_at, u.handle, rv.model, rv.verification, rv.rerun_reason, rv.trusted FROM reviews rv JOIN users u ON u.id = rv.user_id WHERE rv.return_id = $1 ORDER BY rv.id`, [r.id]);
