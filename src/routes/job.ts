...
job.post("/result", bearer, project, async (req: any, res) => {
  ...
  const warnings = [...(effortNote ? [effortNote] : []), ...(returnLogWarn ? [returnLogWarn] : []), ...onceWarning(tokens), ...patchWarning, ...ledgerWarn, ...omissionWarn, ...twinWarn, ...(stray.length ? [`recipe_md names ${stray.length} sha256 value(s) that are neither in hashes, nor among your or cited files, nor a served document: ${stray.map((x: string) => x.slice(0, 12) + "…").join(", ")}. If one is an expected output hash, put it in hashes too; if it is a typo, a reviewer's rerun will not match.`] : [])];
...
async function resubmitTranscript(req: any, res: any, kind: "return" | "review"): Promise<void> {
  ...
  res.json({ ok: true, [kind]: id, tokens, log: tokens.log, effort_evidence: effortEvidence, warnings: onceWarning(tokens), note: `${kind} #${id} now carries this log; ${ttot.toLocaleString("en-US")} tokens counted and credited to your person${kind === "return" ? "; the public transcript URL refreshes within an hour" : ""}.` });
}
