...
job.post("/result", bearer, project, assignmentMutation(async (req: any, res) => {
  ...
  if (jobRow?.type === "review" || (!jobRow && b.type === "review")) {
    ...
    if (Array.isArray(b.also_fix)) {
      ...
      // A trusted reviewer's fix is a job (Chris, Sep 12 2026): one audit job per served file, for whoever comes next.
      if (fixes.length && reviewerTrusted) {
        ...
        for (const f of fixes) await spawnFixJob(Number(parentRow.problem_id), parentRow.lane_id === null || parentRow.lane_id === undefined ? null : Number(parentRow.lane_id), req.project.slug, f.path!, f.note, { reviewId: Number(rv?.id ?? 0) || null, returnId: reviewOf, handle: req.user!.handle });
      }
    }
 ...
 */
export async function spawnFixJob(problemId: number, laneId: number | null, slug: string, path: string, note: string, by: { reviewId?: number | null; returnId?: number | null; handle?: string | null }): Promise<number | null> {
  ...
  const open = await one<{ id: string }>(`SELECT id FROM jobs WHERE problem_id = $1 AND type = 'audit' AND title = $2 AND status IN ('queued','assigned')`, [problemId, title]);
  if (open) return Number(open.id);
  const P = "<project base>";
  const brief = `A reviewer found a defect in the served file \`${rel}\`${by.reviewId ? ` while reviewing return #${by.returnId} (review #${by.reviewId}${by.handle ? ` by @${by.handle}` : ""})` : ""}. Fix it; do not redo the work it belongs to.
