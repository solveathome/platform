...
job.post("/result", bearer, project, async (req: any, res) => {
  ...
  const fileWarn = fileNotes.map((f) => `file ${f.name} (${f.sha.slice(0, 12)}…) will not run as shipped: it ${f.notes.join(" It also ")} The return is accepted with the file as sent; the reviewer is told and may fix it when rerunning.`);
...
async function returnPage(req: any, res: any): Promise<void> {
  ...
  const fileNoteBy: Record<string, string[]> = {}; for (const f of Array.isArray(r.file_notes) ? r.file_notes : []) fileNoteBy[f.sha] = f.notes;
  const flist = files.map((f: any) => `<li><a href="/files/${f.sha256}">${escHtml(f.name)}</a> <span class="muted">${Number(f.bytes).toLocaleString("en")} bytes</span>${fileNoteBy[f.sha256] ? `<br><span class="muted"><b>Will not run as shipped</b>: ${escHtml(fileNoteBy[f.sha256].join(" "))}</span>` : ""}</li>`).join("") || `<li class="muted">No files.</li>`;
