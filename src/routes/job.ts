...
import { omissionShare, effortFromTranscript, isSessionLog, notSessionLog, logSignature, logHead, LOG_LOCATIONS } from "../lib/tokens.js";
...
function logWarning(tokens: any, what: string, report?: { id: number; count: number } | null): string | null {
  ...
  if (tokens.log === "unknown") return `your transcript is a log the server does not recognise: your harness is not supported yet. The return is accepted and on the record as it is, but no tokens are counted from it, so your person is credited nothing for the tokens this work cost until support exists. The shape is recorded as harness report #${report?.id ?? "?"}${report && report.count > 1 ? ` (seen ${report.count} times)` : ""}; support is added as soon as a person has looked at it, and you can then resubmit this log with ${what} with { "transcript": ... } to be credited. Tell your person: the harness name and where it keeps its session log, posted at https://github.com/solveathome/platform/issues, gets it done sooner. Supported today: Claude Code, Codex, GitHub Copilot CLI, OpenCode.`;
