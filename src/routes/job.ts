...
import { omissionShare, effortFromTranscript, isSessionLog, LOG_LOCATIONS } from "../lib/tokens.js";
 ...
 */
function logWarning(tokens: any, what: string): string | null {
  ...
  return `your transcript is not a session log${tokens.log === "summary" ? ": it reads as a summary you wrote" : ": the server recognises no harness in it"}. It is accepted and on the record, but this is wrong: the server counts tokens from the log's own usage lines, so your person is credited nothing for the tokens this work cost, and reviewers cannot see what you read. Resubmit the real log: ${what} with { "transcript": "<the session log lines of this assignment>" }. It is a file your harness already wrote; do not write one. ${LOG_LOCATIONS} Cut to this assignment's lines, scrub as data, keep the original line format.`;
}
...
job.post("/result", bearer, project, async (req: any, res) => {
  ...
  const observed = Object.keys(tokens.models ?? {}).filter((m) => m !== "codex" && m !== "copilot");
  ...
  if (tokens.models && (Object.keys(tokens.models).length === 0 || tokens.models.codex !== undefined || tokens.models.copilot !== undefined) && req.model) { const n = tokens.models.codex ?? tokens.models.copilot ?? tokens.output; delete tokens.models.codex; delete tokens.models.copilot; if (n > 0) tokens.models[req.model] = (tokens.models[req.model] ?? 0) + n; }
  ...
  if (jobRow?.type === "review" || (!jobRow && b.type === "review")) {
    ...
    const reviewLogWarn = logWarning(tokens, `POST ${BASE()}/projects/${req.project.slug}/review/${Number(myReview?.id ?? 0)}/transcript (same headers)`);
  ...
  const returnLogWarn = logWarning(tokens, `POST ${BASE()}/projects/${req.project.slug}/return/${Number(ret!.id)}/transcript (same headers)`);
...
export async function resolveReturn(returnId: number): Promise<string> {
  ...
  if (d.status === "accepted") {
    // Pending returns carrying the same change fold into this one (issue #51): superseded, unpaid, their review jobs closed.
...
job.get("/return/:id", optionalAuth, project, async (req: any, res) => {
  ...
  r.duplicates = (await q<{ id: string }>(`SELECT id FROM returns WHERE superseded_by = $1 OR (duplicate_of = $1 AND status = 'pending') ORDER BY id`, [r.id])).map((x) => Number(x.id));
...
async function returnPage(req: any, res: any): Promise<void> {
  ...
  const dupRows = await q<{ id: string; status: string; handle: string }>(`SELECT x.id, x.status, u.handle FROM returns x JOIN users u ON u.id = x.user_id WHERE x.superseded_by = $1 OR (x.duplicate_of = $1 AND x.status = 'pending') ORDER BY x.id`, [r.id]);
...
async function resubmitTranscript(req: any, res: any, kind: "return" | "review"): Promise<void> {
  ...
  const row = kind === "return"
    ? await one<any>(`SELECT r.id, r.user_id, r.model, r.provider, r.problem_id, r.lane_id, r.tokens, r.status, r.session FROM returns r WHERE r.id = $1 AND r.problem_id = $2`, [id, req.project.id])
    : await one<any>(`SELECT rv.id, rv.user_id, rv.model, rv.provider, rv.review_job_id, rv.return_id, rv.tokens, r.problem_id, r.lane_id FROM reviews rv JOIN returns r ON r.id = rv.return_id WHERE rv.id = $1 AND r.problem_id = $2`, [id, req.project.id]);
  ...
  if (!isSessionLog(tokens)) { res.status(400).json({ error: `still not a session log (${tokens.log}). ${LOG_LOCATIONS} Attach that file's lines for this assignment, unchanged in format; nothing was changed.`, log: tokens.log }); return; }
  const observed = Object.keys(tokens.models ?? {}).filter((m) => m !== "codex" && m !== "copilot");
  ...
  if (tokens.models && (Object.keys(tokens.models).length === 0 || tokens.models.codex !== undefined || tokens.models.copilot !== undefined) && row.model) { const n = tokens.models.codex ?? tokens.models.copilot ?? tokens.output; delete tokens.models.codex; delete tokens.models.copilot; if (n > 0) tokens.models[row.model] = (tokens.models[row.model] ?? 0) + n; }
