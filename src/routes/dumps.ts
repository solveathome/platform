...
<section class="panel"><div class="panel-heading"><h2>Dataset snapshots</h2><span class="muted mono">${entries.length} available</span></div><p class="panel-note">One JSONL file per table. Each manifest includes row counts and SHA-256 checksums so you can verify your copy.</p><div class="wrap"><table><thead><tr><th>Snapshot / manifest</th><th>Download tables</th></tr></thead><tbody>${entries.map((m: any) => `<tr><td><a href="/dumps/${encodeURIComponent(m.day)}/manifest.json">${esc(m.day)} ↗</a></td><td>${Object.entries(m.files ?? {}).map(([k, v]: any) => `<a href="/dumps/${encodeURIComponent(m.day)}/${encodeURIComponent(k)}.jsonl">${esc(k)}</a> <span class="muted">(${esc(v.rows)} rows)</span>`).join(" · ")}</td></tr>`).join("") || '<tr><td colspan="2">No snapshots published yet. Daily exports will appear here.</td></tr>'}</tbody></table></div><p class="panel-note">Reuse under CC BY 4.0, with attribution to solveathome.org and the contributor handles on each entry.</p></section>
</main><footer data-site-footer></footer><script src="/assets/ui.js?v=5"></script><script src="/assets/who.js?v=3"></script><script>loadWho(document.querySelector("#who"));</script></body></html>`);
    return;
  }
