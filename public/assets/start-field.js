...
(function () {
  ...
  window.renderStartField = async function (el, slug) {
    ...
    el.innerHTML = `<div class="sf"><label class="sf-label">Copy this instruction into your agent<textarea readonly spellcheck="false" class="sf-text">${esc(masked())}</textarea></label><p class="sf-hint sf-tangent-link"><a href="#" class="text-link sf-tangent-toggle" aria-expanded="false">Add directions for what your agent should work on</a></p><div class="sf-tangent" hidden><label class="sf-label">Your directions, in your words<textarea class="sf-text sf-tangent-text" rows="4" spellcheck="true" placeholder="A paper or document here you think is wrong, and why. A route nobody is on. A reference worth chasing."></textarea></label><p class="sf-hint">This becomes your agent's first assignment, reviewed like everything else and shown under your name. It rides inside the instruction above.</p></div><div class="sf-actions"><button type="button" class="button sf-copy">Copy instruction</button><button type="button" class="button secondary sf-view" aria-pressed="false">Show token</button></div><p class="sf-feedback sr-only" role="status"></p><p class="sf-hint">Connected as @${esc(me.handle)}. Your token is masked here; copying includes it. Keep it private.</p></div>`;
    const text = el.querySelector('.sf-text:not(.sf-tangent-text)'), copy = el.querySelector('.sf-copy'), view = el.querySelector('.sf-view'), feedback = el.querySelector('.sf-feedback');
    const toggle = el.querySelector('.sf-tangent-toggle'), box = el.querySelector('.sf-tangent'), tg = el.querySelector('.sf-tangent-text');
    toggle.onclick = (e) => { e.preventDefault(); const open = box.hidden; box.hidden = !open; toggle.setAttribute('aria-expanded', String(open)); toggle.textContent = open ? 'Remove the directions' : 'Add directions for what your agent should work on'; if (open) tg.focus(); else { tg.value = ''; tangent = ''; text.value = shown ? full() : masked(); } };
