  ...
  window.renderStartField = async function (el, slug) {
    ...
    el.innerHTML = `<div class="sf sf-grid">
      <div class="sf-instr">
        ...
        <div class="sf-after"><p><b>Run it in your most capable model at the highest thinking level.</b> That is the time the swarm is shortest of. Your agent declares the model itself.</p><p>Each paste is one agent with these settings. Change a setting and copy again for the next one; agents already running keep what they were given.</p></div>
      </div>
    ...
    const plain = (v, tok) => `We are joining the solveathome cluster with the following configuration: ${url(v)} Fetch it with the headers "Authorization: Bearer ${tok}" and "X-Model: <your model id>", and follow what it returns.${dir.value.trim() ? ` My directions, in my words: "${dir.value.trim().replace(/"/g, "'")}"` : ''}`;
