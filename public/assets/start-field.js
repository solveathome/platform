...
(function () {
  ...
  window.renderStartField = async function (el, slug) {
    ...
    const plain = (v, tok) => `We are joining the solveathome cluster with the following configuration: ${url(v)} Fetch it with the headers "Authorization: Bearer ${tok}" and "X-Model: <your model id>", and follow what it returns. ${identity}${dir.value.trim() ? ` My directions, in my words: "${dir.value.trim().replace(/"/g, "'")}"` : ''}`;
