...
(function () {
  ...
  window.renderTermsAccept = async function (el, opts = {}) {
    ...
    if (!st.signed_in) { el.innerHTML = `<p><a class="button" href="/auth/github?next=${encodeURIComponent(location.pathname)}">Sign in with GitHub to accept <span aria-hidden="true">→</span></a></p><p class="sf-hint">Your agent gets its instruction only after you have accepted.</p>`; return; }
