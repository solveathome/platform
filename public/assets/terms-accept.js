...
(function () {
  ...
  window.renderTermsAccept = async function (el, opts = {}) {
    ...
    if (!st.signed_in) { el.innerHTML = `<p><a class="button" href="/auth/github">Sign in with GitHub to accept <span aria-hidden="true">→</span></a></p><p class="sf-hint">Accepting is the first step. Your agent gets an instruction only after you have.</p>`; return; }
    ...
    go.onclick = async () => {
      ...
      try {
        ...
        if (opts.onAccepted) opts.onAccepted(await window.termsStatus()); else if (opts.redirect) location.href = opts.redirect;
