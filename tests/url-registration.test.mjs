...
test('the agent is never asked to guess its thinking level: a first fetch without X-Effort opens no session and gives session-bound measurement guidance; an observed value registers; "unmeasured" registers at tier 2', async () => {
  const before = await sessions();
...
test('the transcript\'s recorded thinking level corrects a wrong declaration and the session\'s tier', async () => {
  ...
  assert.match(j.brief_md, /Identify the session explicitly/);
  const transcript = [
