...
async function ecosystem(w,{rounds}) {
  ...
  for(let round=0;round<rounds;round++) {
    for(const actor of shuffle([author,frontier,judge,worker],w.rng)) {
      ...
      if(a.research_stage==='first_look') {
  ...
  const forward=frontierHours.filter(x=>['discover','first_look','pursue'].includes(x.stage)).reduce((n,x)=>n+x.hours,0);
  ...
  for(const route of routes.values()) {
    if(['known','transfer'].includes(route.kind))assert.equal(route.pursuits,0,'These scripted first-look failures must not get pursuit funding.');
...
async function lateChallenge(w) {
  ...
  await w.take(author,a=>a.research_stage==='first_look');
...
async function interruptions(w) {
  ...
  assert.equal(a.held.research_stage,'first_look');const interrupted=a.held;
