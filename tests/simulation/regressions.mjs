...
export async function transitiveEvidence(w) {
  ...
  await w.take(a,j=>j.research_stage==='first_look');const first=await a.submit(advance(ar.research.route_id,'A1'));
  ...
  await w.take(b,j=>j.research_stage==='first_look');const bEvidence=await b.submit(advance(br.research.route_id,'B1'));
  ...
  await w.take(c,j=>j.research_stage==='first_look');await c.submit(advance(cr.research.route_id,'C1'));
...
export async function withdrawnReceipt(w) {
  ...
  await w.take(author,j=>j.research_stage==='first_look');await author.submit(advance(route.research.route_id,'Dependent1'));
...
export async function freshFirstLook(w) {
  const a=await w.actor('author','claude-opus-5');
  const old=await a.submit({type:'direction',research:proposal('Established')});
  await w.take(a,j=>j.research_stage==='first_look');await a.submit(advance(old.research.route_id,'Established1'));
  const fresh=await a.submit({type:'direction',research:proposal('Fresh')});
  let seen=false,pursuits=0;
  for(let i=0;i<4;i++){
    const job=await w.take(a,j=>['first_look','pursue'].includes(j.research_stage));
    if(job.research_stage==='first_look'){assert.equal(Number(job.research_route_id),fresh.research.route_id);seen=true;break;}
    pursuits++;await a.submit(advance(old.research.route_id,`Established${i+2}`));
  }
  assert.ok(seen,'A fresh eligible lead must receive a first look after at most three pursuits.');
  await a.submit(advance(fresh.research.route_id,'Fresh1'));
  w.record('expectations',{pursuits_before_fresh_first_look:pursuits});
}
...
export async function firstAcceptance(w) {
  ...
  await w.take(author,j=>j.research_stage==='first_look');
...
export async function opusResearch(w) {
  ...
  for(let turn=0;turn<40;turn++) {
    ...
    if(job.research_stage==='first_look') {
  ...
  // Even after the review backlog grows, fresh self-assigned proposals can still get a first look.
...
export async function priorWorkFirst(w) {
  ...
  w.record('expectations',{published_count_not_recomputed:true,prior_work_stops_first_look_and_pursuit:true,no_automatic_review_or_rescue:true,search_record_reused:true,uncovered_extension_allowed:true,source_findings_are_scripted:true});
}
export async function guidanceDelivery(w) {
  ...
  await w.take(author,j=>j.research_stage==='first_look');
  await inspect(author,'first_look',/A first look is the route’s first bounded test/);
...
export const regressions={transitiveEvidence,withdrawnReceipt,waitingCheck,duplicateClaims,freshFirstLook,lateConflict,firstAcceptance,opusResearch,opusRescue,priorWorkFirst,guidanceDelivery};
