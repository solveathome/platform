...
Your job is to make the derivation fail. Check, against the book's Theorem 9.1 (pp. 103 to 112) and Definition 1.3, eq. (1.5), p. 8 (the pages are not served here: `attestation/EXTERNAL-SOURCES.md` names the source, and `attestation/book-ch5-6/` redirects to the publisher; work from the published statement or a public restatement and say which), that the density condition Omega(kappa) with omega(2)=1, omega(p)=2 is satisfied with the exact quantifier the book uses (all pairs 2 <= w1 < w), that the remainder weight 2 sum 4^nu(m)|r_A(m)| is absorbed by z^(eps/2) as section 3 of the note claims, and that the sieve sifts p < z with z = p_n + 1 rather than z = p_n. Then check the finite side: run `node research/exact-g2-ladder.js` and confirm every exact G2 value through 43# sits below the bound's shape (it must; the bound is asymptotic and this only catches a wrong exponent direction).
