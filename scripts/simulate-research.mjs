...
function save() {
  ...
  const rows=report.runs.map(r=>{
    const h=r.snapshot?.hours??[],hours=h.reduce((n,x)=>n+x.hours,0),forward=h.filter(x=>['discover','first_look','pursue','rescue'].includes(x.stage)).reduce((n,x)=>n+x.hours,0);
  ...
  const research=report.runs.filter(r=>r.scenario==='ecosystem').map(r=>{
    const f=(r.snapshot?.hours??[]).filter(x=>x.tier===1),total=f.reduce((n,x)=>n+x.hours,0),forward=f.filter(x=>['discover','first_look','pursue'].includes(x.stage)).reduce((n,x)=>n+x.hours,0),v=r.snapshot?.verification;
