...
ssh "$SERVER" 'flock -w 600 /run/lock/solveathome-deploy.lock sh -c "
  ...
  for i in \$(seq 1 30); do sleep 2; if docker compose -f docker-compose.prod.yml exec -T backend wget -qO- http://localhost:8600/healthz 2>/dev/null | grep -q ok; then echo healthy; exit 0; fi; done
